...
class Scheduler:
    ...
    def __repr__(self):
        formatted_string = [
            ...
            f"initial_wds: {self.initial_wds}\n",
    ...
    def record_lrs(self, param_groups):
        ...
        self.initial_wds = [g["weight_decay"] for g in param_groups]

    def update(self, coef: float):
        assert self.initial_lrs is not None, "Please run .record_lrs(optimizer) first."
        for curr_group, init_lr, init_wd in zip(self.optimizer.param_groups, self.initial_lrs, self.initial_wds):
            curr_group["lr"] = coef * init_lr

            if self.scheduler_cfg.get("schedule_wd", False):
                curr_group["weight_decay"] = coef * init_wd
