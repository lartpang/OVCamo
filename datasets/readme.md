...
For convenience, we provide the images in several links including Google Drive and Data Repository, but we do not own the copyright of the images.
It is solely your responsibility to check the original licenses of the images before using them.
Any use of the images are at your own discretion and risk.
Furthermore, please refer to the following dataset description on how to set up OVCamo before running our code.
...
1. Unzip all the files and set the following path information in [the yaml file `dataset.yaml`](./dataset.yaml):
...
2. Download from the information json for OVCamo:
   1. [`class_info.json`](https://github.com/lartpang/OVCamo/releases/download/dataset-v1.0/class_info.json)
   2. [`sample_info.json`](https://github.com/lartpang/OVCamo/releases/download/dataset-v1.0/sample_info.json)
3. Specify the data roots for training and testing, and run the script to split the dataset: `python split_dataset.py --root-info dataset.yaml --class-info class_info.json --sample-info sample_info.json --train_root TRAIN_ROOT --test_root TEST_ROOT`
4. [Optional] Depth maps of the training set which are generated by us, can be downloaded from <https://github.com/lartpang/OVCamo/releases/download/dataset-v1.0/OVCOS-TrDepth.zip>
5. Use the training and testing splits of dataset in the training and inference processes.
