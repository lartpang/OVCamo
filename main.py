...
class ImageTestDataset(data.Dataset):
    def __init__(self, dataset_info: Config, shape: dict):
        super().__init__()
        self.shape = shape
    ...
    def __getitem__(self, index):
        ...
        image = ops.resize(image, height=self.shape["h"], width=self.shape["w"])
...
class ImageTrainDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        depth = ops.resize(depth, height=base_h, width=base_w)
...
def test(model, cfg, metric_names=("sm", "wfm", "mae", "fm", "em", "iou")):
    te_dataset = ImageTestDataset(dataset_info=cfg.root_info, shape=cfg.datasets.test.shape)
    ...
    for batch in tqdm(te_loader, total=len(te_loader), ncols=79, desc="[EVAL]"):
        ...
        for idx_in_batch, pred in enumerate(probs):
            ...
            mask_array = io.read_gray_array(mask_path.as_posix(), thr=0)
...
def train(model, cfg):
    tr_dataset = ImageTrainDataset(dataset_info=cfg.root_info, shape=cfg.datasets.train.shape)
